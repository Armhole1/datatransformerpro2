...
import { Receipt, UploadCloud, CheckCircle, AlertCircle, Loader2, X, Download } from 'lucide-react';
import { extractDocumentData } from '../lib/gemini';
import { RECEIPT_PROMPT, RECEIPT_SCHEMA } from '../lib/schemas';
...
export default function ReceiptScanner() {
  ...
  const exportData = (format: 'csv' | 'xlsx') => {
    ...
    try {
      ...
      const exportData = successfulFiles.flatMap((f) => {
        const { lineItems, ...receiptData } = f.data;
        if (lineItems && lineItems.length > 0) {
          ...
          return [{ fileName: f.file.name, ...receiptData }];
        }
      });
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <div className="mb-8">
        <h2 className="text-2xl font-bold text-gray-900 mb-2">Receipt Scanner</h2>
        <p className="text-gray-600">
          Upload receipts (PDF or Images) to extract structured data for expense tracking.
        </p>
      </div>
                      ...
                      {file.status === 'success' && file.data && (
                        <>
                          ...
                          <span className="text-xs text-gray-600 truncate">
                            {file.data.merchantName || 'Unknown Merchant'} •{' '}
                            {file.data.date || 'No Date'} • $
