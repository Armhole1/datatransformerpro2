...
export default function BankStatementParser() {
  ...
  const exportData = (format: 'csv' | 'xlsx') => {
    ...
    try {
      ...
      const exportData = successfulFiles.flatMap((f) => {
        const { transactions, ...statementData } = f.data;
        if (transactions && transactions.length > 0) {
          ...
          return [{ fileName: f.file.name, ...statementData }];
        }
      });
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <div className="mb-8">
        <h2 className="text-2xl font-bold text-gray-900 mb-2">Bank Statement Parser</h2>
        <p className="text-gray-600">
          Upload bank statements (PDF or Images) to extract transactions for reconciliation.
        </p>
      </div>
                      ...
                      {file.status === 'success' && file.data && (
                        <>
                          ...
                          <span className="text-xs text-gray-600 truncate">
                            {file.data.bankName || 'Unknown Bank'} •{' '}
                            Acct: {file.data.accountNumber || 'N/A'} • Bal: $
                            {file.data.endingBalance || '0.00'}
                          </span>
                        </>
                      )}
                      {file.status === 'error' && (
